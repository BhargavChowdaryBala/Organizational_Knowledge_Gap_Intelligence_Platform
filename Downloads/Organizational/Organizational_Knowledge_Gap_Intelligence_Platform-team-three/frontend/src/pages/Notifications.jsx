...
const SectionTitle = ({ title, action, actionText, onActionClick }) => (
  <div className="flex justify-between items-center mb-6">
    ...
    {action && (
      <button onClick={onActionClick} className="text-indigo-600 dark:text-indigo-400 text-xs font-semibold hover:underline cursor-pointer">
        {actionText || 'View All'}
      </button>
    )}
  </div>
...
const Notifications = () => {
  ...
  // Helper to map backend notifications to UI format
  const mapBackendNotification = (n) => {
    let iconBg = 'bg-slate-100 dark:bg-slate-700';
    let iconColor = 'text-slate-600 dark:text-slate-300';
    let icon = <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0" /></svg>;
    let actionText = '';
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (res && res.data && res.data.length > 0) {
          setItems(res.data.map(mapBackendNotification));
        } else {
          setItems(notificationsList);
        }
      } catch (err) {
        console.warn('Backend notifications fetch failed, using fallback mock list:', err);
        setItems(notificationsList);
      } finally {
        setLoading(false);
      }
  ...
  const markAllRead = async () => {
    const unreadItems = items.filter(n => n.unread && n.id);
    ...
    <div className="p-8 max-w-[1600px] mx-auto bg-slate-50/50 dark:bg-transparent min-h-screen">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Upcoming Reminders */}
          <Card>
            <SectionTitle title="Upcoming Reminders" action={true} actionText="View Calendar" onActionClick={() => setShowCalendarModal(true)} />
            ...
            <button 
              ...
              className="w-full mt-4 py-3 text-indigo-600 dark:text-indigo-400 text-xs font-bold flex items-center justify-center gap-2 hover:bg-slate-50 dark:hover:bg-white/5 rounded-lg transition-colors group cursor-pointer"
            >
              View Full Calendar 
      ...
      {showCalendarModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-2xl w-full border border-slate-200 dark:border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Reminders & Assessment Calendar</h3>
                <p className="text-xs text-slate-500">Upcoming deadlines, scheduled assessments, and reminders</p>
              </div>
              <button 
            ...
            <div className="bg-indigo-50 dark:bg-indigo-500/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-500/20 mb-6 flex justify-between items-center">
              ...
              <span className="text-xs font-semibold px-2.5 py-1 rounded bg-indigo-600 text-white">Full Schedule</span>
            </div>

            <div className="space-y-4 mb-6">
              {reminders.map((rem, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-800/40">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-lg bg-indigo-600 text-white flex flex-col items-center justify-center font-bold text-xs shrink-0">
                      <span>{rem.day}</span>
                      <span className="text-[9px] uppercase">{rem.month}</span>
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-sm">{rem.title}</h4>
                      <p className="text-xs text-slate-500">{rem.dateStr}</p>
                    </div>
                  </div>
                  <span className={`px-2.5 py-1 rounded text-xs font-bold ${rem.timeColor}`}>
                    {rem.time}
                  </span>
                </div>
              ))}
            ...
            <div className="flex justify-end">
              <button 
                onClick={() => setShowCalendarModal(false)}
                className="px-5 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 cursor-pointer"
              >
                Close Calendar
              </button>
            </div>
