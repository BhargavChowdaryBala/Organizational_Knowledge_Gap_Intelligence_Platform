...
const SectionTitle = ({ title, action, actionText, onActionClick }) => (
  <div className="flex justify-between items-center mb-6">
    ...
    {action && (
      <button onClick={onActionClick} className="text-indigo-600 dark:text-indigo-400 text-sm font-medium hover:underline cursor-pointer">
        {actionText || 'View All'}
      </button>
    )}
  </div>
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-3 rounded-lg shadow-xl text-xs font-bold text-slate-800 dark:text-white">
        <p className="font-extrabold text-indigo-600 dark:text-[#d9f95d] mb-1">{label || payload[0]?.payload?.subject}</p>
...
const topicQuizQuestionsMap = {
  'React.js': [
    { id: 1, question: "What is the primary function of React's Virtual DOM?", options: ["Direct DOM manipulation", "Reconciliation & high performance UI rendering", "Database indexing", "CSS styling"], answer: 1 },
    { id: 2, question: "Which hook is used to manage side effects in functional components?", options: ["useState", "useContext", "useEffect", "useReducer"], answer: 2 },
    { id: 3, question: "How do you pass data from parent to child component in React?", options: ["Props", "State", "Redux", "Services"], answer: 0 }
  ],
  'Java': [
    { id: 1, question: "Which annotation marks a class as a Spring REST Controller?", options: ["@Component", "@RestController", "@Service", "@Bean"], answer: 1 },
    { id: 2, question: "What is Dependency Injection in Java Spring Boot?", options: ["Inverting control to inject dependencies automatically", "Writing raw SQL queries", "Compiling Java bytecode", "Creating thread pools"], answer: 0 },
    { id: 3, question: "Which feature was introduced in Java 17 LTS?", options: ["Sealed Classes & Pattern Matching", "Generics", "Lambda expressions", "Annotations"], answer: 0 }
  ],
  'SQL': [
    { id: 1, question: "Which SQL clause is used to filter aggregated group records?", options: ["WHERE", "HAVING", "GROUP BY", "ORDER BY"], answer: 1 },
    { id: 2, question: "What type of JOIN returns matching records from both tables?", options: ["LEFT JOIN", "INNER JOIN", "RIGHT JOIN", "CROSS JOIN"], answer: 1 },
    { id: 3, question: "Which PostgreSQL feature improves lookup speeds on large tables?", options: ["Indexes (B-Tree/GIN)", "Stored procedures", "Views", "Triggers"], answer: 0 }
  ],
  'SQL & Postgres': [
    { id: 1, question: "Which SQL clause is used to filter aggregated group records?", options: ["WHERE", "HAVING", "GROUP BY", "ORDER BY"], answer: 1 },
    { id: 2, question: "What type of JOIN returns matching records from both tables?", options: ["LEFT JOIN", "INNER JOIN", "RIGHT JOIN", "CROSS JOIN"], answer: 1 },
  ...
  'Communication': [
    { id: 1, question: "What is the key element of active listening during team discussions?", options: ["Interrupting early", "Reflecting and summarizing speaker points", "Multitasking on email", "Avoiding eye contact"], answer: 1 },
    { id: 2, question: "How should technical blockers be communicated to stakeholders?", options: ["Using high-level business impact and clear timelines", "Using complex jargon", "Ignoring stakeholder messages", "Sending raw log files"], answer: 0 },
    { id: 3, question: "What is the recommended approach for constructive peer feedback?", options: ["Public criticism", "Specific, actionable feedback focused on outcomes", "Vague hints", "Delayed review"], answer: 1 }
  ],
  'Problem Solving': [
    { id: 1, question: "What is the time complexity of binary search on a sorted array?", options: ["O(N)", "O(log N)", "O(N^2)", "O(1)"], answer: 1 },
    { id: 2, question: "Which data structure follows First-In, First-Out (FIFO) ordering?", options: ["Stack", "Queue", "Binary Tree", "Heap"], answer: 1 },
    { id: 3, question: "What algorithmic technique breaks a problem into smaller subproblems?", options: ["Greedy Search", "Dynamic Programming", "Brute Force", "Depth-First Search"], answer: 1 }
  ]
};
...
const SkillAssessment = () => {
  ...
  const [quizSkill, setQuizSkill] = useState('React.js');
  const [quizType, setQuizType] = useState('Self');
  const [currentQuestions, setCurrentQuestions] = useState(topicQuizQuestionsMap['React.js']);
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [userAnswers, setUserAnswers] = useState({});
  const [quizCompleted, setQuizCompleted] = useState(false);
  const [quizScore, setQuizScore] = useState(0);
  const [historyList, setHistoryList] = useState(recentAssessments);

  const tabs = ['Overview', 'Self Assessment', 'Peer Assessment', 'Assessment History', 'Scheduled Assessments'];

  // Dynamic statistics calculated from current historyList
  const totalAssessmentsCount = historyList.length;
  const selfCount = historyList.filter(h => h.type === 'Self').length;
  const peerCount = historyList.filter(h => h.type === 'Peer').length;
  const numericScores = historyList.map(h => parseInt(h.score) || 0);
  ...
  const skillsAssessedCount = new Set(historyList.map(h => h.skill)).size;

  const startQuiz = (skill = 'React.js', type = 'Self') => {
    setQuizSkill(skill);
    setQuizType(type);
    const qList = topicQuizQuestionsMap[skill] || topicQuizQuestionsMap[skill.split(' ')[0]] || topicQuizQuestionsMap['React.js'];
  ...
  const handleFinishQuiz = () => {
    ...
    const finalScore = Math.round((scoreCount / currentQuestions.length) * 100);
    ...
    const newRecord = {
      ...
      levelColor: finalScore >= 80 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700',
      status: 'Completed'
    };
    setHistoryList(prev => [newRecord, ...prev]);
  };

  const allSkills = Object.keys(topicQuizQuestionsMap);
  ...
  <span className="font-semibold text-slate-800 dark:text-white">
    Skill Assessment
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <StatCard 
          title="Total Assessments" value={totalAssessmentsCount} subtext="Completed"
        ...
        />
        <StatCard 
          title="Average Score" value={`${avgScore}%`} subtext="Across all skills"
        ...
        />
        <StatCard 
          title="Skills Assessed" value={skillsAssessedCount} subtext="This Year"
      ...
      <div className="flex border-b border-slate-200 dark:border-white/5 mb-8 overflow-x-auto hide-scrollbar">
        {tabs.map((tab) => (
          <button
            ...
            className={`px-6 py-3 text-sm font-medium whitespace-nowrap transition-colors border-b-2 cursor-pointer ${
              activeTab === tab 
      ...
      {activeTab === 'Overview' && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-8">
          ...
          <div className="xl:col-span-2 flex flex-col gap-8">
                ...
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                      <PolarGrid stroke="#475569" strokeDasharray="2 2" />
                      <PolarAngleAxis dataKey="subject" tick={<CustomRadarTick />} />
                      <PolarRadiusAxis angle={30} domain={[0, 100]} tick={false} axisLine={false} />
                      <Radar name="Required" dataKey="required" stroke="#94a3b8" strokeWidth={2} strokeDasharray="3 3" fill="transparent" />
                      <Radar name="Current" dataKey="current" stroke="#6366f1" strokeWidth={2.5} fill="#6366f1" fillOpacity={0.45} />
              ...
              <div className="w-full md:w-64 shrink-0 border-t md:border-t-0 md:border-l border-slate-100 dark:border-white/5 pt-6 md:pt-0 md:pl-8 flex flex-col justify-center">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">{selfCount}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-500 dark:text-slate-400">Peer Assessments</span>
                    <span className="font-bold text-slate-900 dark:text-white">{peerCount}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-100 dark:border-white/5">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">{avgScore}%</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">{lowestScore}%</span>
                  </div>
                  <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-100 dark:border-white/5">
                    <span className="text-slate-500 dark:text-slate-400">Total Skills Assessed</span>
                    <span className="font-bold text-slate-900 dark:text-white">{skillsAssessedCount}</span>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-0 overflow-hidden">
              <div className="p-6 border-b border-slate-200 dark:border-white/5 flex justify-between items-center">
                ...
                <button onClick={() => setActiveTab('Assessment History')} className="text-indigo-600 dark:text-indigo-400 text-sm font-medium hover:underline cursor-pointer">View All</button>
              </div>
                    ...
                    {recentAssessments.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button 
                            onClick={() => startQuiz(item.skill, item.type === 'Peer' ? 'Peer' : 'Self', item)} 
                            title="Retake or View Assessment"
          ...
          <div className="flex flex-col gap-8">
            <Card>
              <SectionTitle title="Upcoming Assessments" action={true} onActionClick={() => setActiveTab('Scheduled Assessments')} />
              <div className="space-y-4">
                {upcomingAssessments.map((assessment, idx) => (
                  <div key={idx} className="border border-slate-100 dark:border-white/5 rounded-xl p-4 hover:shadow-md transition-shadow dark:bg-white/[0.02]">
                    ...
                    <button 
                      onClick={() => startQuiz(assessment.skill, assessment.type.includes('Self') ? 'Self' : 'Peer')}
                      className={`w-full py-2 border rounded-lg text-sm font-medium transition-colors cursor-pointer ${assessment.actionClass || 'border-indigo-200 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-500/30 dark:text-indigo-400 dark:hover:bg-indigo-500/10'}`}
                    >
            ...
            <Card>
              <SectionTitle title="Performance Trend" action={true} actionText="View Report" onActionClick={() => setActiveTab('Assessment History')} />
              <div className="h-[200px] w-full mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Tooltip content={<CustomTooltip />} />
                    <Line type="monotone" dataKey="score" stroke="#8b5cf6" strokeWidth={3} dot={{ r: 4, fill: '#8b5cf6', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 6 }} label={{ position: 'top', fill: '#cbd5e1', fontSize: 10 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card className="relative overflow-hidden bg-gradient-to-br from-white to-slate-50 dark:from-[#1a202c] dark:to-[#151a23]">
              ...
              <div className="space-y-5 relative z-10 w-2/3">
                <button onClick={() => setActiveTab('Self Assessment')} className="flex items-start gap-3 text-left w-full cursor-pointer">
                ...
                <button onClick={() => setActiveTab('Peer Assessment')} className="flex items-start gap-3 text-left w-full cursor-pointer">
                ...
                <button onClick={() => setActiveTab('Assessment History')} className="flex items-start gap-3 text-left w-full cursor-pointer">
                ...
                <button onClick={() => setActiveTab('Scheduled Assessments')} className="flex items-start gap-3 text-left w-full cursor-pointer">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Scheduled Assessments</h4>
                    <p className="text-[10px] text-slate-500">View your scheduled assessments</p>
                  </div>
                </button>
              </div>
      ...
      {activeTab === 'Self Assessment' && (
        <div className="mb-8">
          <Card className="mb-6">
            <h3 className="font-bold text-slate-900 dark:text-white text-lg mb-2">Self Assessment</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Evaluate your own skills and knowledge. Select a skill to start a self assessment.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {allSkills.map(skill => (
                <div key={skill} className="border border-slate-100 dark:border-white/5 rounded-xl p-5 flex flex-col items-start gap-3 hover:shadow-md transition-shadow dark:bg-white/[0.02]">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-indigo-50 text-indigo-500 dark:bg-indigo-500/10 flex items-center justify-center shrink-0">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.25 9.75L16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z" /></svg>
                    </div>
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm">{skill}</h4>
                  </div>
                  <p className="text-xs text-slate-500">{topicQuizQuestionsMap[skill].length} questions · Self Evaluation</p>
                  <button 
                    onClick={() => startQuiz(skill, 'Self')}
                    className="w-full mt-auto py-2 border border-indigo-200 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-500/30 dark:text-indigo-400 dark:hover:bg-indigo-500/10 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
                  >
      ...
      {activeTab === 'Peer Assessment' && (
        <div className="mb-8">
          <Card className="mb-6">
            <h3 className="font-bold text-slate-900 dark:text-white text-lg mb-2">Peer Assessment</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Get feedback from your peers and colleagues. Request or complete a peer assessment.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border border-slate-100 dark:border-white/5 rounded-xl p-5">
                <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm mb-2">Request Peer Assessment</h4>
                <p className="text-xs text-slate-500 mb-4">Send a peer assessment request to a colleague for a specific skill.</p>
                <select className="w-full p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-300 mb-3">
                  {allSkills.map(skill => <option key={skill} value={skill}>{skill}</option>)}
                </select>
                <button 
                  onClick={() => startQuiz('React.js', 'Peer')}
                  className="w-full py-2 border border-orange-200 text-orange-600 hover:bg-orange-50 dark:border-orange-500/30 dark:text-orange-400 dark:hover:bg-orange-500/10 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
                >
                  Send Request
                </button>
              </div>
              <div className="border border-slate-100 dark:border-white/5 rounded-xl p-5">
                <h4 className="font-semibold text-slate-800 dark:text-slate-200 text-sm mb-2">Complete Peer Assessment</h4>
                <p className="text-xs text-slate-500 mb-4">You have 3 pending peer assessment requests awaiting your review.</p>
                {['React.js', 'SQL', 'Communication'].map((skill, i) => (
                  <div key={skill} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-white/5 last:border-0">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded bg-orange-50 text-orange-500 dark:bg-orange-500/10 flex items-center justify-center text-xs font-bold">
                        {skill.charAt(0)}
                      </div>
                      <span className="text-sm text-slate-700 dark:text-slate-300">{skill}</span>
                    </div>
                    <button 
                      onClick={() => startQuiz(skill, 'Peer')}
                      className="text-xs px-3 py-1.5 rounded-lg border border-orange-200 text-orange-600 hover:bg-orange-50 dark:border-orange-500/30 dark:text-orange-400 dark:hover:bg-orange-500/10 font-semibold cursor-pointer"
                    >
                      Review
                    </button>
                  </div>
      ...
      {activeTab === 'Assessment History' && (
        <div className="mb-8">
          <Card className="p-0 overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-white/5 flex justify-between items-center">
              <h3 className="font-bold text-slate-900 dark:text-white text-lg">All Assessment History</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
      ...
      {activeTab === 'Scheduled Assessments' && (
        <div className="mb-8">
          <Card className="mb-6">
            <h3 className="font-bold text-slate-900 dark:text-white text-lg mb-2">Scheduled Assessments</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Upcoming assessments scheduled for you.</p>
            <div className="space-y-4">
              {upcomingAssessments.map((assessment, idx) => (
                <div key={idx} className="border border-slate-100 dark:border-white/5 rounded-xl p-4 hover:shadow-md transition-shadow dark:bg-white/[0.02] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${assessment.iconColor}`}>
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.25 9.75L16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z" /></svg>
                    </div>
                  ...
                  <button 
                    onClick={() => startQuiz(assessment.skill, assessment.type.includes('Self') ? 'Self' : 'Peer')}
                    className="px-4 py-2 rounded-lg text-sm font-semibold transition-colors cursor-pointer border border-indigo-200 text-indigo-600 hover:bg-indigo-50 dark:border-indigo-500/30 dark:text-indigo-400 dark:hover:bg-indigo-500/10"
                  >
                    {assessment.action || 'Start Assessment'}
                  </button>
                </div>
      ...
      {showQuizModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 max-w-lg w-full border border-slate-200 dark:border-white/10 shadow-2xl">
            {!quizCompleted ? (
              <div>
                <div className="flex justify-between items-center mb-4">
                  <span className={`text-xs font-bold px-2.5 py-1 rounded ${quizType === 'Peer' ? 'bg-orange-50 text-orange-600 dark:bg-orange-500/10' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10'}`}>
                    {quizSkill} · {quizType} Assessment
                  </span>
                ...
                <div className="space-y-2 mb-6">
                  {currentQuestions[currentQuestion].options.map((opt, oIdx) => (
                    <button
                      key={oIdx}
                      onClick={() => handleSelectOption(currentQuestion, oIdx)}
                      className={`w-full p-3 rounded-lg text-xs text-left font-medium border transition-all cursor-pointer ${
                        userAnswers[currentQuestion] === oIdx
                          ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-500/20 text-indigo-900 dark:text-indigo-200'
                ...
                <div className="flex justify-between">
                  <button
                    ...
                    className="px-4 py-2 bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300 rounded-lg text-xs font-bold hover:bg-slate-200 transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                  {currentQuestion < currentQuestions.length - 1 ? (
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 transition-colors cursor-pointer"
                    >
                      Next Question
                    </button>
                  ) : (
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition-colors cursor-pointer"
                    >
                      Submit Assessment
                    </button>
                  )}
              ...
              <div className="text-center py-4">
                <div className="w-16 h-16 rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 flex items-center justify-center mx-auto mb-4 text-2xl font-bold">
                  ✓
                </div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Assessment Completed!</h3>
                <p className="text-sm text-slate-500 mb-4">Your {quizType.toLowerCase()} score for {quizSkill} is <span className="font-extrabold text-emerald-600">{quizScore}%</span>.</p>
                <button
                  onClick={() => setShowQuizModal(false)}
                  className="px-6 py-2.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700 cursor-pointer"
                >
                  Back to Dashboard
                </button>
              </div>
