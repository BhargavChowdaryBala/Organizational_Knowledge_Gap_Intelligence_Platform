...
const KnowledgeGapAnalysis = () => {
  ...
  const [showGapSummaryModal, setShowGapSummaryModal] = useState(false);
        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-6">
              ...
              <Card>
                ...
                {viewMode === 'bars' ? (
                  ...
                  <div className="overflow-x-auto py-2">
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">Competency Gap Intensity Matrix across skills & role levels:</p>
                    <div className="grid grid-cols-5 gap-3 min-w-[500px]">
                      {gaps.map((g, idx) => {
                        const gapVal = g.gap || (g.expectedLevel - g.currentLevel) || 0;
                        const bgClass = gapVal >= 3 
                          ? 'bg-rose-50 border-rose-200 text-rose-700 dark:bg-rose-500/25 dark:text-rose-300 dark:border-rose-500/40' 
                          : gapVal === 2 
                          ? 'bg-orange-50 border-orange-200 text-orange-700 dark:bg-orange-500/25 dark:text-orange-300 dark:border-orange-500/40' 
                          : gapVal === 1 
                          ? 'bg-amber-50 border-amber-200 text-amber-700 dark:bg-amber-500/25 dark:text-amber-300 dark:border-amber-500/40' 
                          : 'bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-500/25 dark:text-emerald-300 dark:border-emerald-500/40';
                        return (
                          <div key={idx} className={`p-3 rounded-xl border flex flex-col items-center justify-center text-center transition-all hover:scale-105 cursor-pointer shadow-sm ${bgClass}`}>
                            <span className="text-xs font-bold truncate w-full mb-1">{g.skillName}</span>
                            <span className="text-lg font-black">{gapVal > 0 ? `-${gapVal}` : '✓ OK'}</span>
                            <span className="text-[10px] font-semibold opacity-90">{g.currentLevel}/{g.expectedLevel || g.targetLevel || 5}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div className="flex flex-col gap-6">
              {/* Gap Summary Donut */}
              <Card>
                <SectionTitle title="Gap Summary" action={true} actionText="View All" onAction={() => setShowGapSummaryModal(true)} />
      ...
      {showGapSummaryModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-3xl w-full border border-slate-200 dark:border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white">Complete Skill Gap Summary</h3>
                <p className="text-xs text-slate-500">Full analysis breakdown across all evaluated competencies</p>
              </div>
              <button 
                onClick={() => setShowGapSummaryModal(false)}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div className="p-4 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-center">
                ...
                <p className="text-xs font-bold text-red-700 dark:text-red-300 mt-1">Critical Gaps</p>
              </div>
              <div className="p-4 rounded-xl bg-orange-50 dark:bg-orange-500/10 border border-orange-100 dark:border-orange-500/20 text-center">
                <span className="text-2xl font-black text-orange-600 dark:text-orange-400">{highCount}</span>
                <p className="text-xs font-bold text-orange-700 dark:text-orange-300 mt-1">High Gaps</p>
              </div>
              <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 text-center">
                ...
                <p className="text-xs font-bold text-amber-700 dark:text-amber-300 mt-1">Medium Gaps</p>
              </div>
              <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 text-center">
                <span className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{lowCount}</span>
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300 mt-1">No Gaps</p>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 text-slate-500 uppercase tracking-wider">
                    <th className="py-3 px-4 font-bold">Skill Name</th>
                    <th className="py-3 px-4 font-bold">Current Level</th>
                    <th className="py-3 px-4 font-bold">Required Level</th>
                    <th className="py-3 px-4 font-bold">Gap Size</th>
                    <th className="py-3 px-4 font-bold">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {gaps.map((item, idx) => {
                    const gapVal = item.gap || (item.expectedLevel - item.currentLevel);
                    return (
                      <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                        <td className="py-3 px-4 font-bold text-slate-800 dark:text-slate-200">{item.skillName}</td>
                        <td className="py-3 px-4 font-semibold text-slate-600 dark:text-slate-400">{item.currentLevel} / 5</td>
                        <td className="py-3 px-4 font-semibold text-slate-600 dark:text-slate-400">{item.expectedLevel} / 5</td>
                        <td className="py-3 px-4 font-extrabold text-slate-900 dark:text-white">{gapVal > 0 ? `-${gapVal}` : '0'}</td>
                        <td className="py-3 px-4">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold ${
                            gapVal >= 3 ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400' :
                            gapVal === 2 ? 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400' :
                            gapVal === 1 ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
                            'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'
                          }`}>
                            {gapVal >= 3 ? 'Critical' : gapVal === 2 ? 'High' : gapVal === 1 ? 'Medium' : 'Mastered'}
                          </span>
                        </td>
                      </tr>
                    );
            ...
            <div className="flex justify-end">
              <button 
                onClick={() => setShowGapSummaryModal(false)}
                className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg cursor-pointer"
              >
                Close Summary
              </button>
            </div>
